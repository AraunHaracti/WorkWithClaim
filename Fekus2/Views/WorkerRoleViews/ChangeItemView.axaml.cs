...
public partial class ChangeItemView : Window
{
    ...
    public ChangeItemView(Action action, WorkerRole item)
    {
        InitializeComponent();
    ...
    {
        string sql = $"insert into WorkerRole (Name) " +
                     $"values ('{_item.Name}')";
    ...
    {
        string sql = $"update WorkerRole as t set " +
                     $"t.Name = '{_item.Name}' " +
                     $"where t.RoleID = '{_item.RoleId}'";
